/*
 ...
 Будьте внимательны при обновлении словаря:
 если одновременно в одном выражении вычислять размер словаря и добавлять в него новый элемент,
...
int main() {
	...
	for (int i = 0; i < Q; i++) {
		...
		if (busStops.count(stops) == 0) {
			int busNumber = busStops.size() + 1;
			busStops[stops] = busNumber;
